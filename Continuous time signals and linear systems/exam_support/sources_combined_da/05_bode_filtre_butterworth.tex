...
\symref{Hjw}{\(H(\jj\omega)\)}{frekvensrespons}{afhænger af system}
\symref{HdB}{\(H_{\mathrm{dB}}\)}{amplitudestørrelse i decibel}{dB}
\symref{phase}{\(\angle H\)}{fase af frekvensrespons}{grader eller rad}
\symref{K}{\(K\)}{statisk eller skalerende forstærkning}{afhænger af system}
\symref{wc}{\(\omega_c\)}{knæk- eller cutoff-vinkelfrekvens}{rad/s}
\symref{fp}{\(f_p\)}{pasbåndsfrekvens}{Hz}
\symref{fs}{\(f_s\)}{stopbåndsfrekvens}{Hz}
\symref{Ap}{\(A_p\)}{tilladt pasbåndsdæmpning}{dB}
\symref{As}{\(A_s\)}{krævet stopbåndsdæmpning}{dB}
\symref{n}{\(n\)}{filterorden eller harmonisk indeks efter kontekst}{dimensionsløs}
...
\textbf{Symboler:} \(|H|_{\mathrm{dB}}\) er amplitudeforhold i dB; \(\angle H\) er fase; \(\omega\) skal være i rad/s.
...
\textbf{Symboler:} \(z_i\) er nulpunkter; \(p_k\) er poler; \(K\) er samlet forstærkning.
...
\textbf{Symboler:} \(n\) er filterorden; \(\epsilon\) bestemmer pasbåndsrippleparameteren; \(\omega_p\) er pasbåndsgrænsen.
...
\textbf{Symboler:} \(A_p\) og \(A_s\) er positive dB-dæmpninger; \(\omega_p,\omega_s\) skal bruge samme enhed.
...
\textbf{Symboler:} \(S_x^y\) er sensitivitet af \(y\) med hensyn til komponent eller parameter \(x\).
