...
\symref{h}{\(h(t)\)}{impulsrespons}{outputenhed per inputenhed per s}
\symref{H}{\(H(s)\)}{overføringsfunktion i Laplace-domænet}{afhænger af system}
\symref{p}{\(p_k\)}{pol nummer \(k\)}{1/s}
\symref{z}{\(z_i\)}{nulpunkt nummer \(i\)}{1/s}
\symref{zeta}{\(\zeta\)}{dæmpningsfaktor}{dimensionsløs}
\symref{omegan}{\(\omega_n\)}{udæmpet egenvinkelfrekvens}{rad/s}
\symref{omegad}{\(\omega_d\)}{dæmpet egenvinkelfrekvens}{rad/s}
\symref{Q}{\(Q\)}{kvalitetsfaktor}{dimensionsløs}
...
\textbf{Symboler:} \(a,b\) er vilkårlige skalarer; \(x_1,x_2\) er testinput; \(y_1,y_2\) er de tilhørende output.
...
\textbf{Symboler:} \(a_k,b_m\) er konstante koefficienter; \(N\) er højeste outputorden; \(M\) er højeste inputorden. \(H(s)\) må kun tolkes som overføringsfunktion ved nul begyndelsesbetingelser.
...
\textbf{Symboler:} \(b_0,b_1,b_2\) bestemmer tællerens frekvensadfærd; \(a_0,a_1\) bestemmer polerne via nævneren.
...
\textbf{Symboler og enheder:} \(R\) i ohm, \(C\) i farad, \(L\) i henry, \(i\) i ampere og \(v\) i volt.
...
\textbf{Hurtigtjek:} \(Z_C\) bliver lille ved høj frekvens; \(Z_L\) bliver stor ved høj frekvens.
...
\textbf{Symboler:} \(y_{\mathrm{zs}}\) er nul-start-respons; \(*\) betyder foldning; \(\tau\) er integrationsvariabel.
...
\textbf{Symboler:} \(\mathcal{L}^{-1}\) betyder invers Laplace-transformation; \(K\) er direkte gennemgangsforstærkning.
...
\textbf{Symboler:} \(\omega_n\) sætter hastighedsskalaen; \(\zeta\) sætter dæmpningen; \(Q\) er stor ved lav dæmpning.
...
\textbf{Hurtigtjek:} negative realdele er stabile; positiv realdel er altid ustabil for kausale LTIC-systemer.
...
\textbf{Symboler:} \(PO\) er procentvist overshoot; \(t_p\) er første top-tid; \(t_s\) er indsvings-/settlingtid.
