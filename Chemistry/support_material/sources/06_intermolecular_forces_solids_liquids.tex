...
\pythontool{\texttt{clausius\_clapeyron\_pressure(...)} naar \(P_2\) skal beregnes fra to temperaturer og \(\Delta H_{\mathrm{vap}}\)}
...
\pythontool{\texttt{unit\_cell\_volume(...)} naar densitet og \(Z\) skal omsaettes til cellevolumen}
...
\pythontool{\texttt{freezing\_point\_depression(...)} til frysepunktssaenkning med solut, solventmasse og van't Hoff-faktor}
...
\pythontool{\texttt{freezing\_point\_depression(...)} som kontrol for \(m\), \(\Delta T_f\) og frysepunkt; omarranger stadig manuelt naar den ukendte er molarmassen}
