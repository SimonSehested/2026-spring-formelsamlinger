...
class December2025ReuseTests(unittest.TestCase):
    def test_freezing_point_comparison_q15(self):
        nacl = 2 * 1.0 * (1 / 58.44) / 0.500
        cacl2 = 3 * 1.0 * (1 / 110.98) / 0.500
        self.assertGreater(nacl, cacl2)
    ...
    def test_common_ion_bromide_solubility_q18(self):
        silver_s = solubility_from_ksp(3.3e-13, (1, 1), (0, 1.0e-6))
        copper_s = solubility_from_ksp(5.3e-9, (1, 1), (0, 1.0e-6))
        self.assertLess(silver_s * molar_mass("AgBr"), 0.010)
        self.assertLess(copper_s * molar_mass("CuBr"), 0.100)
