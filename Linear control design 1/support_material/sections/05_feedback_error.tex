...
For systemtype og statisk loopgain samles open-loop først som én rational funktion.
Type er antal poler i origo. For type \(n\) er den relevante statiske konstant
\[
K_0=\lim_{s\to0}s^nL(s).
\]
Eksempel: \(1+K_2/s=(s+K_2)/s\) indeholder stadig én integrator, så en controller som
\(K_1(1+K_2/s)\) er en PI-controller.
...
\code{closed\_loop\_characteristic(forward,feedback,s)} bruges, når forward- og
feedbackgrenene er skrevet symbolsk, og du vil danne den karakteristiske ligning.
\code{closed\_loop\_analysis\_from\_coefficients(num,den,K)} bruges, når du har
koefficienter for \(G(s)\) og vil have karakteristisk polynomium, poler, stabilitet og
closed-loop DC-gain. \code{closed\_loop\_poles(num,den,K)} er den korte pol-liste.
...
Når en disturbance \(d\) spørger til output eller fejl, skal reference sættes til nul,
og DC-ligningerne skrives for disturbance-pathen. Referencefejlformlen kan ikke
genbruges ukritisk, fordi disturbance-fortegnet ofte ændrer fejlsignalets fortegn.
...
\code{steady\_state\_error\_analysis(num,den,proportional\_gain,input\_type)} bruges,
når standard negativ unity feedback er bekræftet, og du vil have systemtype,
fejlkonstanter \(K_p^\ast,K_v,K_a\), closed-loop-poler og stationær fejl for step,
ramp eller parabel. Brug den ikke på disturbance- eller sensorfejl uden først at have
reduceret signalvejen til den tilsvarende standardform.
