...
\(r\), \(R(s)\), \(R\) & Reference i tid/Laplace-domæne. \(R(s)=1/s\) for unit step og \(R(s)=h_0/s\) for step med størrelse \(h_0\). & Systemspecifik & \S\ref{sec:feedback}, \S\ref{sec:disturbance}\\
\(e\), \(E(s)\), \(E\) & Fejl mellem reference og målt output; fx \(e=r-Hy\). & Systemspecifik & \S\ref{sec:feedback}, \S\ref{sec:disturbance}\\
\(u\), \(U(s)\), \(U\) & Kontrolsignal/plantinput. I modelafsnit er \(U(s)\) Laplace-transformeret input. & Systemspecifik & \S\ref{sec:model}, \S\ref{sec:feedback}, \S\ref{sec:limits}\\
\(u_{\mathrm{lin}}(t)\) & Ubegrænset lineær controllerkommando før saturation/rate limit. & Som \(u\) & \S\ref{sec:limits}\\
...
\(y\), \(Y(s)\), \(Y\) & Output i tid/Laplace-domæne. I modelafsnit er \(Y(s)\) Laplace-transformeret output. & Systemspecifik & \S\ref{sec:model}, \S\ref{sec:feedback}, \S\ref{sec:disturbance}\\
...
\(G(s)\), \(G\) & Plant eller generel transferfunktion. Bruges også i \(G=Y/U\). & Ratio & \S\ref{sec:model}, \S\ref{sec:bode}, \S\ref{sec:feedback}, \S\ref{sec:pilead}, \S\ref{sec:disturbance}, \S\ref{sec:feedforward}\\
...
\(C(s)\), \(C\) & Controller. & Ratio & \S\ref{sec:feedback}, \S\ref{sec:pilead}, \S\ref{sec:limits}, \S\ref{sec:feedforward}\\
\(C_0(s)\), \(C_0\) & Controllerdelen uden \(K_P\), brugt når \(K_P\) isoleres fra magnitudebalancen. & Ratio & \S\ref{sec:pilead}\\
\(C_{PI}\) & PI-controllerform/genkendelsessignal. & Ratio & \S\ref{sec:pilead}\\
\(C_{\mathrm{lead}}\) & Lead-controllerform/genkendelsessignal. & Ratio & \S\ref{sec:pilead}\\
...
\(F_d(s)\), \(F_d\) & Disturbance feed-forward-blok. & Ratio & \S\ref{sec:disturbance}, \S\ref{sec:feedforward}\\
\(L(s)\), \(L\) & Loop transfer function, typisk \(CG\) eller \(CGH\) afhængigt af sensor. & Ratio & \S\ref{sec:bode}, \S\ref{sec:feedback}, \S\ref{sec:disturbance}\\
\(S(s)\), \(S\) & Sensitivity \(1/(1+L)\). & Ratio & \S\ref{sec:feedback}, \S\ref{sec:disturbance}\\
\(T(s)\), \(T\) & Complementary sensitivity \(L/(1+L)\). & Ratio & \S\ref{sec:disturbance}\\
\multicolumn{4}{@{}l}{\textbf{Laplace, polynomier og stabilitet}}\\
\(s\) & Laplacevariabel. Bruges også som polynomiumsvariabel i scripts. & \(\mathrm{s}^{-1}\) & \S\ref{sec:model}, \S\ref{sec:feedback}, \S\ref{sec:pilead}, \S\ref{sec:limits}, \S\ref{sec:feedforward}\\
...
\(\omega\) & Vinkelfrekvens. & rad/s & \S\ref{sec:model}, \S\ref{sec:bode}, \S\ref{sec:pilead}\\
\(p\) & Pol eller karakteristisk polynomium afhængigt af kontekst; \(p(s,K)\) er karakteristisk polynomium med gain. & \(\mathrm{s}^{-1}\) eller polynomium & \S\ref{sec:model}\\
...
\(K\) & Generelt realt gain eller parameter i karakteristisk polynomium. & Varierer & \S\ref{sec:model}\\
\(K_{\mathrm{ss}}\) & DC/static gain \(\lim_{s\to0}G(s)\). & output/input & \S\ref{sec:model}, \S\ref{sec:time}\\
\(\LHP\) & Left half-plane; poler med negativ realdel. & Område & \S\ref{sec:model}, \S\ref{sec:feedback}, \S\ref{sec:feedforward}\\
\(\RHP\) & Right half-plane; poler med positiv realdel. & Område & \S\ref{sec:model}, \S\ref{sec:bode}, \S\ref{sec:nyquist}\\
\(P\) & Antal open-loop RHP-poler i Nyquistkriteriet. & Antal & \S\ref{sec:nyquist}\\
...
\(\omega_n\) & Naturlig frekvens for andenordensstandardform. & rad/s & \S\ref{sec:time}\\
\(\zeta\) & Damping ratio. & -- & \S\ref{sec:time}\\
\(M_p\) & Overshoot som ratio; procent fås ved at gange med 100. & -- eller \% & \S\ref{sec:time}\\
\(t_r\) & Rise time. & s & \S\ref{sec:time}\\
\(t_s\) & Settling time, her typisk 2\%-approksimation. & s & \S\ref{sec:time}\\
...
\(\omega_c\) & Gain crossover frequency, hvor loopmagnitude er 1. & rad/s & \S\ref{sec:bode}, \S\ref{sec:pilead}, \S\ref{sec:limits}\\
\(\omega_\pi\) & Phase crossover frequency, hvor fasen er \(-180^\circ\). & rad/s & \S\ref{sec:bode}\\
\(\gamma_M\) & Phase margin. & grader & \S\ref{sec:bode}, \S\ref{sec:pilead}, \S\ref{sec:limits}\\
...
\(K_P\) & Proportionalgain. & -- eller ratio & \S\ref{sec:bode}, \S\ref{sec:feedback}, \S\ref{sec:pilead}\\
...
\(\tau_i\) & PI-/Lag-tid; \(N_i=\omega_c\tau_i\). & s & \S\ref{sec:pilead}, \S\ref{sec:limits}\\
...
\(\tau_d\) & Lead-tid. & s & \S\ref{sec:pilead}\\
\(\alpha\) & Lead-forhold; skal opfylde \(0<\alpha<1\). & -- & \S\ref{sec:pilead}, \S\ref{sec:limits}\\
\(N_i\) & Dimensionsløs PI-zero-placering \(N_i=\omega_c\tau_i\). & -- & \S\ref{sec:pilead}, \S\ref{sec:limits}\\
...
\(\beta\) & Lag-forhold; skal opfylde \(\beta>1\). & -- & \S\ref{sec:limits}\\
...
\(\sigma_D\) & Fortegn for disturbancebidrag ved summering; \(\sigma_D\in\{-1,+1\}\). & -- & \S\ref{sec:feedforward}\\
...
\(\operatorname{atan2}\) & Kvadrantkorrekt vinkelfunktion. & Funktion & \S\ref{sec:bode}, \S\ref{sec:pilead}\\
...
\(\pi\), \(\infty\), \(\%\), \(^\circ\) & Matematisk konstant, uendelighed, procent og grader. & Konstanter/enheder & \S\ref{sec:time}, \S\ref{sec:bode}, \S\ref{sec:pilead}\\
\(<,>,\leq,\geq\) & Ulighedstegn til stabilitetsintervaller, parameterkrav og actuatorlimits. & Relationer & \S\ref{sec:model}, \S\ref{sec:limits}\\
