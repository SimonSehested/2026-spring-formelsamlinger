...
\code{design\_lag(lag\_phase\_deg,n\_i,omega\_c,tau\_i,beta)} bruges, når du vil have
mere end \(\beta\): funktionen kan beregne eller checke lagfasen, finde \(\tau_i\),
\(N_i\), lag-zero/pole-frekvenser og warnings. Den vælger ikke den manglende fase for
dig; fasebalancen skal stadig udledes fra opgaven.
...
\code{feedforward\_analysis(...,disturbance\_sign)} bruges, når den samme ideelle
feed-forward også skal vurderes for poler, nulpunkter, relative degree, strict
properness, minimum-phase og realiserbarhed. Funktionen kan ikke afgøre om disturbance
er målelig eller om fortegnet er korrekt; det er stadig diagramlæsning.
